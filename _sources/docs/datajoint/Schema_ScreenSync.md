...
**Type**: `dj.Computed`  **Depends on**: `UnitySession` (requires `TeensyData` and `ScreenTTLData`)
...
| `nr_barcodes_detected` | int32 | Number of barcodes detected by the Teensy (from lux sensor decoding) |
...
2. **Detected** — The Teensy reads the BH1750 lux sensor at high frequency; the `decode_session_lux_dj()` utility decodes the lux trace to identify barcode bursts
...
# Sessions with poor sync quality
poor_sync = (ScreenSync & "fraction_barcodes_matched < 0.9").fetch(as_dict=True)
